...
const sellCryptoUSDTScene = new Scenes.WizardScene(
	'sellCryptoUSDTTypeScene', // Уникальный идентификатор сцены
	(ctx) => {
...
`, { parse_mode: 'HTML' })
		...
		if(ctx.callbackQuery.data === 'uah') {
			...
			try {
				...
				btcPrice = parseFloat(data.find(item => item.symbol === 'USDTUAH').price).toFixed(2);
...
module.exports = sellCryptoUSDTScene;
