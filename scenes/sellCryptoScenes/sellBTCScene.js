...
const sellCryptoBTCScene = new Scenes.WizardScene(
	'sellCryptoBTCTypeScene', // Уникальный идентификатор сцены
	(ctx) => {
	...
	async (ctx) => {
		if (ctx.callbackQuery.data === 'usd') {
			...
			try {
				...
				const data = await response.json()
				btcPrice = parseFloat(data.find(item => item.symbol === 'BTCUSDT').price).toFixed(2);
...
module.exports = sellCryptoBTCScene;
