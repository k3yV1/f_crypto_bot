...
const buyCryptoETHScene = new Scenes.WizardScene(
	'buyCryptoETHTypeScene', // Уникальный идентификатор сцены
	(ctx) => {
...
💳 <b>Покупка</b>: ETH
💰 <b>Цеза за ${cryptoAmount} ETH</b>: ${price} USD
...
`)
		...
		if(ctx.callbackQuery.data === 'uah') {
			...
			try {
				...
				const data = await response.json()
				btcPrice = parseFloat(data.find(item => item.symbol === 'ETHUAH').price).toFixed(2);
				console.log('btc: ', btcPrice)
			} catch (e) {
				console.log(e)
			}
...
👤 <b>Пользователь</b>: @${ctx.from.username}
💳 <b>Покупка</b>: ETH
💰 <b>Цеза за ${cryptoAmount} ETH</b>: ${price} UAH
💱 <b>Валюта</b>: ${currency.toUpperCase()}`, { parse_mode: 'HTML'});
...
module.exports = buyCryptoETHScene;
